...
class UnitsController extends Controller
{
    ...
    public function store(Unit $unit)
    {
        ...
        } else if ($user->units->contains($unit->id)) {
    ...
    public function remove(Unit $unit)
    {
        ...
        if (!$unit) {
            ...
            \Session::flash('status', 'User does not have this unit !');
        } else {
            \Session::flash('status', 'You have successfully sold ' . $unit->name . ' for ' . ($unit->gold / 2) . ' gold.');
            $user->units()->detach($unit->id);
            $user->gold += ($unit->gold / 2);
            $user->save();
        }
